...
Generate a starter config and edit it to add your Azure key, endpoint,
and deployment name:

```bash
pixforge init
# wrote starter config to /Users/you/.config/pixforge/config.toml
...
to `~/.config/pixforge/config.toml` on every OS (use `pixforge config-path` to
...
```bash
pixforge -p "Cyberpunk Tokyo street at night, neon, rain"
```
...
```bash
pixforge -p "watercolor mountains" -m MAI-Image-2e -W 1024 -H 1024
```
